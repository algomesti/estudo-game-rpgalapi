...
class Session {
    ...
    public function createSession($chars) {
        ...
        $player1['race_name'] = $chars[0]['race']['name'];
        ...
        $player2['race_name'] = $chars[1]['race']['name'];
    ...
    private function verifyResultShift($token) {
        $session = $this->getSession($token);
        $life_p1 = $session['player1']['life'];
        $life_p2 = $session['player2']['life'];
        if($life_p1==0 || $life_p2 ==0) {
            $arr['status'] = false;
            $this->save($token, $arr, 2000);
        }
    }
    
    private function verifyStatus($token) {
        $ses = $this->getSession($token);
        return $ses['shift']['status'];
   }
